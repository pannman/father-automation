...
from ..config.text.habatakesakimono_text_config import HabatakesakimonoText
from .fc2 import Fc2
import datetime


class Habatakesakimono(Fc2):
    def login_id(self):
        return LOGIN.HABATAKESAKIMONO_LOGIN['ID']

    def login_pass(self):
        return LOGIN.HABATAKESAKIMONO_LOGIN['PASS']
    ...
    def get_main_sign_gold(self, zone):
        ...
        if CONFIG.habatakesakimono_main_buy_result_gold(zone) == "売り":
        ...
        if CONFIG.habatakesakimono_main_buy_result_gold(zone) == "買い":
    ...
    def get_main_sign_platinum(self, zone):
        ...
        if CONFIG.habatakesakimono_main_buy_result_platinum(zone) == "売り":
        ...
        if CONFIG.habatakesakimono_main_buy_result_platinum(zone) == "買い":
    ...
    def get_total_platinum_file(self, zone):
        if zone == "日中":
            self.main_total_platinum = open(
                'other_txt/habatakesakimono/habatakesakimono_day_main_total_platinum.txt', 'r').read()
            self.main_total_platinum = 0 if self.main_total_platinum == "±0" else int(
                self.main_total_platinum)
        if zone == "夜間":
            self.main_total_platinum = open(
                'other_txt/habatakesakimono/habatakesakimono_yakan_main_total_platinum.txt', 'r').read()
            self.main_total_platinum = 0 if self.main_total_platinum == "±0" else int(
                self.main_total_platinum)
        if zone == "日通し":
            self.main_total_platinum = open(
                'other_txt/habatakesakimono/habatakesakimono_hitoshi_main_total_platinum.txt', 'r').read()
            self.main_total_platinum = 0 if self.main_total_platinum == "±0" else int(
                self.main_total_platinum)

    def save_total_platinum_file(self, zone):
        if zone == "日中":
            open('other_txt/habatakesakimono/habatakesakimono_day_main_total_platinum.txt',
                 'w').write(str(self.main_total_platinum))
        if zone == "夜間":
            open('other_txt/habatakesakimono/habatakesakimono_yakan_main_total_platinum.txt',
                 'w').write(str(self.main_total_platinum))
        if zone == "日通し":
            open('other_txt/habatakesakimono/habatakesakimono_hitoshi_main_total_platinum.txt',
                 'w').write(str(self.main_total_platinum))

    def get_total_gold_file(self, zone):
        if zone == "日中":
            self.main_total_gold = open(
                'other_txt/habatakesakimono/habatakesakimono_day_main_total_gold.txt', 'r').read()
            self.main_total_gold = 0 if self.main_total_gold == "±0" else int(
                self.main_total_gold)
        if zone == "夜間":
            self.main_total_gold = open(
                'other_txt/habatakesakimono/habatakesakimono_yakan_main_total_gold.txt', 'r').read()
            self.main_total_gold = 0 if self.main_total_gold == "±0" else int(
                self.main_total_gold)
        if zone == "日通し":
            self.main_total_gold = open(
                'other_txt/habatakesakimono/habatakesakimono_hitoshi_main_total_gold.txt', 'r').read()
            self.main_total_gold = 0 if self.main_total_gold == "±0" else int(
                self.main_total_gold)

    def save_total_gold_file(self, zone):
        if zone == "日中":
            open('other_txt/habatakesakimono/habatakesakimono_day_main_total_gold.txt',
                 'w').write(str(self.main_total_gold))
        if zone == "夜間":
            open('other_txt/habatakesakimono/habatakesakimono_yakan_main_total_gold.txt',
                 'w').write(str(self.main_total_gold))
        if zone == "日通し":
            open('other_txt/habatakesakimono/habatakesakimono_hitoshi_main_total_gold.txt',
                 'w').write(str(self.main_total_gold))
    ...
    def automation(self, num):
        ...
        if num == 3:
            ...
            habatakesakimono_text = HabatakesakimonoText(zone, CONFIG.habatakesakimono_main_buy_result_gold(zone), CONFIG.habatakesakimono_main_buy_result_platinum(zone), self.main_sign_gold, self.main_total_gold,
            ...
            self.blog_post(self.category_num, habatakesakimono_text, zone, self.will_year,
