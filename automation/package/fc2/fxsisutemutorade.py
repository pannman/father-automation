...
from ..config.text.fxsisutemutorade_text_config import FxsisutemutoradeText
from .fc2 import Fc2
from decimal import Decimal, ROUND_HALF_UP
import datetime


class Fxsisutemutorade(Fc2):
    def login_id(self):
        return LOGIN.FXSISUTEMUTORADE_LOGIN['ID']

    def login_pass(self):
        return LOGIN.FXSISUTEMUTORADE_LOGIN['PASS']
    ...
    def return_will_hour(self, zone):
        if zone == "トレード結果（09：00→16：30）":
            return self.fxsisutemutorade1_will_hour
        if zone == "トレード結果（16：30→09：00）":
            return self.fxsisutemutorade2_will_hour

    def return_will_minute(self, zone):
        if zone == "トレード結果（09：00→16：30）":
            return self.fxsisutemutorade1_will_minute
        if zone == "トレード結果（16：30→09：00）":
            return self.fxsisutemutorade2_will_minute
    ...
    def get_total_file(self, zone):
        if zone == "トレード結果（09：00→16：30）":
            self.main_total = open(
                'other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade1_main_total.txt', 'r').read()
            self.main_total = 0 if self.main_total == "±0" else float(
                self.main_total)
            self.main_total_euro = open(
                'other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade1_main_total_euro.txt', 'r').read()
            self.main_total_euro = 0 if self.main_total_euro == "±0" else float(
                self.main_total_euro)
            self.main_total_lb = open(
                'other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade1_main_total_lb.txt', 'r').read()
            self.main_total_lb = 0 if self.main_total_lb == "±0" else float(
                self.main_total_lb)
        if zone == "トレード結果（16：30→09：00）":
            self.main_total = open(
                'other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade2_main_total.txt', 'r').read()
            self.main_total = 0 if self.main_total == "±0" else float(
                self.main_total)

            self.main_total_euro = open(
                'other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade2_main_total_euro.txt', 'r').read()
            self.main_total_euro = 0 if self.main_total_euro == "±0" else float(
                self.main_total_euro)

            self.main_total_lb = open(
                'other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade2_main_total_lb.txt', 'r').read()
            self.main_total_lb = 0 if self.main_total_lb == "±0" else float(
                self.main_total_lb)
    ...
    def get_main_sign(self, zone):
        ...
        self.zone_state, self.zone_state_euro, self.zone_state_lb = CONFIG.fxsisutemutorade_main_buy_result(
            zone)
    ...
    def save_total_file(self, zone):
        if zone == "トレード結果（09：00→16：30）":
            open('other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade1_main_total.txt',
                 'w').write(str(self.main_total))
            open('other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade1_main_total_euro.txt',
                 'w').write(str(self.main_total_euro))
            open('other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade1_main_total_lb.txt',
                 'w').write(str(self.main_total_lb))
        if zone == "トレード結果（16：30→09：00）":
            open('other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade2_main_total.txt',
                 'w').write(str(self.main_total))
            open('other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade2_main_total_euro.txt',
                 'w').write(str(self.main_total_euro))
            open('other_txt/fxsisutemutorade/fxsisutemutorade_fxsisutemutorade2_main_total_lb.txt',
                 'w').write(str(self.main_total_lb))
    ...
    def __init__(self, driver):
        ...
        self.fxsisutemutorade1_will_hour = "17"
        self.fxsisutemutorade2_will_hour = "10"

        self.fxsisutemutorade1_will_minute = "05"
        self.fxsisutemutorade2_will_minute = "00"
    ...
    def automation(self, num):
        ...
        if num == 1:
            ...
            fxsisutemutorade_text = FxsisutemutoradeText(zone, self.zone_state, self.zone_state_euro, self.zone_state_lb, self.main_sign, self.main_sign_euro, self.main_sign_lb, self.main_total, self.main_total_euro, self.main_total_lb, self.zone_dollar, self.zone_euro, self.zone_lb, self.zone_settlement,
                                             self.zone_settlement_euro, self.zone_settlement_lb, self.buy_time, self.settlement_time)
            self.blog_post(self.category_num, fxsisutemutorade_text, zone,
