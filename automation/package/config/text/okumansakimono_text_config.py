...
class OkumansakimonoText():
    ...
    def blog_text(self):
        return "" + str(CONFIG.result_month()) + "/" + str(CONFIG.result_day()) + "　"+self.zone + "取引結果（億万長者への道）\n\n\n\n"\
            ...
            "売り 　 " + self.main_sign_gold+"　 " + str(CONFIG.result_month()) + "月累計　"+self.main_total_gold +"\n\n\n"\
            "白金（限月2023/2）　\n\n"\
            "始値　"+str(self.start_price_platinum)+"　終値　"+str(self.end_price_platinum)+"\n\n"\
            "売り 　 " + self.main_sign_platinum+"　 " + str(CONFIG.result_month()) + "月累計　"+self.main_total_platinum + "\n\n\n"
