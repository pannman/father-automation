...
class VictoriousfxText():
    def __init__(self, zone, zone_state, zone_state_euro, zone_state_lb, main_sign, main_sign_euro, main_sign_lb, main_total_1, main_total_euro_1, main_total_lb_1, main_total_2, main_total_euro_2, main_total_lb_2, main_total_3, main_total_euro_3, main_total_lb_3, zone_dollar, zone_euro, zone_lb, zone_settlement, zone_settlement_euro, zone_settlement_lb, buy_time, settlement_time):
        ...
        if "+" not in str(main_total_1) and float(main_total_1) > 0  :
            self.main_total_1 ="+"+str(main_total_1)
        else:
            self.main_total_1 = str(main_total_1)
        if "+" not in str(main_total_euro_1) and float(main_total_euro_1) > 0  :
            self.main_total_euro_1 ="+"+str(main_total_euro_1)
        else:
            self.main_total_euro_1 = str(main_total_euro_1)
        if "+" not in str(main_total_lb_1) and float(main_total_lb_1) > 0  :
            self.main_total_lb_1 ="+"+str(main_total_lb_1)
        else:
            self.main_total_lb_1 = str(main_total_lb_1)
        
        if "+" not in str(main_total_2) and float(main_total_2) > 0  :
    
            self.main_total_2 ="+"+str(main_total_2)
        else:
            self.main_total_2 = str(main_total_2)
        if "+" not in str(main_total_euro_2) and float(main_total_euro_2) > 0  :
            self.main_total_euro_2 ="+"+str(main_total_euro_2)
        else:
            self.main_total_euro_2 = str(main_total_euro_2)
        if "+" not in str(main_total_lb_2) and float(main_total_lb_2) > 0  :
            self.main_total_lb_2 ="+"+str(main_total_lb_2)
        else:
            self.main_total_lb_2 = str(main_total_lb_2)

        if "+" not in str(main_total_3) and float(main_total_3) > 0  :
            self.main_total_3 ="+"+str(main_total_3)
        else:
            self.main_total_3 = str(main_total_3)
        if "+" not in str(main_total_euro_3) and float(main_total_euro_3) > 0  :
            self.main_total_euro_3 ="+"+str(main_total_euro_3)
        else:
            self.main_total_euro_3 = str(main_total_euro_3)
        if "+" not in str(main_total_lb_3) and float(main_total_lb_3) > 0  :
        ...
        print("メイン   " + self.main_sign+"    " +
              self.month + "月累計5：30→9：00   " + self.main_total_1 + "9：00→16：30   " + self.main_total_2 + "16：30→5：30   " + self.main_total_3)
        print("ユーロ")
        print(self.buy_euro + "   "+str(self.zone_euro) +
              "   決済    " + str(self.zone_settlement_euro) + "")
        print("メイン   " + self.main_sign_euro+"    " +
              self.month + "月累計5：30→9：00   " + self.main_total_euro_1 + "9：00→16：30   " + self.main_total_euro_2 + "16：30→5：30   " + self.main_total_lb_2)
        print("ポンド")
        print(self.buy_lb + "   "+str(self.zone_lb) +
              "   決済    " + str(self.zone_settlement_lb) + "")
        print("メイン   " + self.main_sign_lb+"    " +
              self.month + "月累計5：30→9：00   " + self.main_total_lb_1 + "9：00→16：30   " + self.main_total_euro_3 + "16：30→5：30   " + self.main_total_lb_3)
    ...
    def blog_text(self):
        return ""+self.zone+"　結果\n\n"\
            "" + self.month + "/" + self.day + "　"+self.zone+"　の有料配信は"+self.buy+"でした。\n\n"\
            ""+self.zone+" ドル/円結果\n\n"\
            ""+self.main_sign+"pips\n\n"\
            ""+self.month+"月度累計　ドル/円結果\n\n"\
            "9：00→16：30 "+self.main_total_2+"pips　\n\n"\
            "16：30→5：30 "+self.main_total_3+"pips　\n\n"\
            "5：30→9：00 "+self.main_total_1+"pips　\n\n\n\n\n\n"\
            ""+self.zone+"　結果\n\n"\
            "" + self.month + "/" + self.day + "　"+self.zone+"　の有料配信は"+self.buy_euro+"でした。\n\n"\
            ""+self.zone+" ユーロ/円結果\n\n"\
            ""+self.main_sign_euro+"pips\n\n"\
            ""+self.month+"月度累計　ユーロ/円結果\n\n"\
            "9：00→16：30 "+self.main_total_euro_2+"pips　\n\n"\
            "16：30→5：30 "+self.main_total_euro_3+"pips　\n\n"\
            "5：30→9：00 "+self.main_total_euro_1+"pips　\n\n\n\n\n\n"\
            ""+self.zone+"　結果\n\n"\
            "" + self.month + "/" + self.day + "　"+self.zone+"　の有料配信は"+self.buy_lb+"でした。\n\n"\
            ""+self.zone+" ポンド/円結果\n\n"\
            ""+self.main_sign_lb+"pips\n\n"\
            ""+self.month+"月度累計　ポンド/円結果\n\n"\
            "9：00→16：30 "+self.main_total_lb_2+"pips　\n\n"\
            "16：30→5：30 "+self.main_total_lb_3+"pips　\n\n"\
            "5：30→9：00 "+self.main_total_lb_1+"pips　\n\n\n\n"\
